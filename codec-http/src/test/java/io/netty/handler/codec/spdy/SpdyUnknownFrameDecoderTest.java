...
public class SpdyUnknownFrameDecoderTest {
    ...
    private final SpdyUnknownFrameDecoderDelegate delegate = mock(SpdyUnknownFrameDecoderDelegate.class);
    ...
    public void createDecoder() {
        decoder = new SpdyFrameDecoder(SpdyVersion.SPDY_3_1, testDelegate) {
            ...
            @Override
            protected boolean decodeUnknownFrame(final int frameType,
                                                 final byte flags,
                                                 final int length,
                                                 final ByteBuf buffer) {
                if (length == 0) {
                    testDelegate.readUnknownFrame(frameType, flags, Unpooled.buffer(0));
                    return true;
                }
                ...
                testDelegate.readUnknownFrame(frameType, flags, data);
                return true;
            }
        };
    }
