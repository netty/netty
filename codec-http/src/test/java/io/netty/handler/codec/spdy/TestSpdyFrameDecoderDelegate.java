...
final class TestSpdyFrameDecoderDelegate implements SpdyUnknownFrameDecoderDelegate {
    ...
    public void readUnknownFrame(final int frameType, final byte flags, final ByteBuf payload) {
        if (delegate instanceof SpdyUnknownFrameDecoderDelegate) {
            ((SpdyUnknownFrameDecoderDelegate) delegate).readUnknownFrame(frameType, flags, payload);
        }
        buffers.add(payload);
    }
