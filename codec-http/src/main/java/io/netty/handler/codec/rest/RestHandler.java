...
public class RestHandler extends SimpleChannelInboundHandler<HttpObject> {
    ...
    protected void setCookies(HttpResponse httpResponse) {
        ...
        String cookieString = request.headers().getAndConvert(HttpHeaderNames.SET_COOKIE);
        if (cookieString != null) {
            Set<Cookie> cookies = CookieDecoder.decode(cookieString);
            httpResponse.headers().add(HttpHeaderNames.SET_COOKIE, ServerCookieEncoder.encode(cookies));
        }
    }
    ...
    protected FullHttpResponse getResponse(ByteBuf content) {
        ...
        if (request == null) {
            ...
            if (content == null) {
                ...
                response.headers().addInt(HttpHeaderNames.CONTENT_LENGTH, content.readableBytes());
            }
            setCookies(response);
            setCloseOnceAnswered(true);
            return response;
