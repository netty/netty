...
public abstract class DualAbstractRouterHandler<T, RouteLike extends DualMethodRouter<T, RouteLike>>
extends SimpleChannelInboundHandler<HttpRequest> {
    private static final byte[] CONTENT_404 = "Not Found".getBytes();

    private final DualMethodRouter<T, RouteLike> router;

    public DualAbstractRouterHandler(DualMethodRouter<T, RouteLike> router) {
        this.router = router;
    }
    ...
    protected void channelRead0(ChannelHandlerContext ctx, HttpRequest req) throws Exception {
        ...
        io.netty.handler.codec.http.routing.Routed<Object> jrouted = router.route(method, qsd.path());
