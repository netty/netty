...
/**
 * Encodes the content of the outbound {@link HttpResponse} and {@link HttpContent}. The original content is replaced
 * with the new content encoded by the {@link EmbeddedChannel}, which is created by {@link #beginEncode(HttpResponse,
 * String)}. Once encoding is finished, the value of the <tt>'Content-Encoding'</tt> header is set to the target content
 * encoding, as returned by {@link #beginEncode(HttpResponse, String)}. Also, the <tt>'Content-Length'</tt> header is
 * updated to the length of the encoded content.  If there is no supported or allowed encoding in the corresponding
 * {@link HttpRequest}'s {@code "Accept-Encoding"} header, {@link #beginEncode(HttpResponse, String)} should return
 * {@code null} so that no encoding occurs (i.e. pass-through).
 * <p>
 * Please note that this is an abstract class.  You have to extend this class and implement {@link
 * #beginEncode(HttpResponse, String)} properly to make this class functional.  For example, refer to the source code of
 * {@link HttpContentCompressor}.
 * <p>
 * This handler must be placed after {@link HttpObjectEncoder} in the pipeline so that this handler can intercept HTTP
 * responses before {@link HttpObjectEncoder} converts them into {@link ByteBuf}s.
 */
public abstract class HttpContentEncoder extends MessageToMessageCodec<HttpRequest, HttpObject> {
    ...
    private static void ensureHeaders(HttpObject msg) {
        checkState(msg instanceof HttpResponse, "unexpected message type: " +
                                                msg.getClass().getName() + " (expected: " +
                                                HttpResponse.class.getSimpleName() + ')');
    }

    private static void ensureContent(HttpObject msg) {
        checkState(msg instanceof HttpContent, "unexpected message type: " +
                                               msg.getClass().getName() + " (expected: " +
     ...
     *
     * @return the result of preparation, which is composed of the determined target content encoding and a new {@link
     * EmbeddedChannel} that encodes the content into the target content encoding. {@code null} if {@code
     * acceptEncoding} is unsupported or rejected and thus the content should be handled as-is (i.e. no encoding).
     */
