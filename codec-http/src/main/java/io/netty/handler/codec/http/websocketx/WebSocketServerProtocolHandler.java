...
public class WebSocketServerProtocolHandler extends WebSocketProtocolHandler {
    ...
    protected void decode(ChannelHandlerContext ctx, WebSocketFrame frame) throws Exception {
        if (serverConfig.handleCloseFrames() && frame instanceof CloseWebSocketFrame) {
            ...
            if (handshaker != null) {
                ...
                ctx.writeAndFlush(Unpooled.EMPTY_BUFFER).addListener(ctx.channel(), ChannelFutureListener.CLOSE);
            }
            return;
        }
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
        if (cause instanceof WebSocketHandshakeException) {
            ...
            ctx.channel().writeAndFlush(response).addListener(ctx.channel(), ChannelFutureListener.CLOSE);
        } else {
