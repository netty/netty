...
public class HttpPostRequestDecoder {
            ...
            while (undecodedChunk.isReadable() && contRead) {
                ...
                case FIELD:// search '&' or end of line
                    if (read == '&') {
                        currentStatus = MultiPartStatus.DISPOSITION;
                        ampersandpos = currentpos - 1;
                        setFinalBuffer(undecodedChunk.slice(firstpos, ampersandpos - firstpos).copy().retain());
                        firstpos = currentpos;
                        contRead = true;
    ...
    private void readFileUploadByteMultipartStandard(String delimiter) throws NotEnoughDataDecoderException,
        ...
        ByteBuf buffer = undecodedChunk.slice(readerIndex, lastPosition - readerIndex).copy().retain();
        if (found) {
            ...
            if (found) {
                ...
                try {
                    currentAttribute.addContent(
                            undecodedChunk.slice(readerIndex, lastPosition - readerIndex).copy().retain(), true);
                } catch (IOException e) {
                    throw new ErrorDataDecoderException(e);
                }
                undecodedChunk.readerIndex(lastPosition);
                ...
                try {
                    currentAttribute.addContent(
                            undecodedChunk.slice(readerIndex, lastPosition - readerIndex).copy().retain(), false);
                } catch (IOException e) {
                    throw new ErrorDataDecoderException(e);
                }
                undecodedChunk.readerIndex(lastPosition);
