 ...
 * <br>
 * <b>Note:</b> implementations of {@link SearchProcessor} scan the {@link io.netty5.buffer.ByteBuf} sequentially,
 * one byte after another, without doing any random access. As a result, when using {@link SearchProcessor}
 * with such methods as {@link io.netty5.buffer.ByteBuf#forEachByte}, these methods return the index of the last byte
 * of the found byte sequence within the {@link io.netty5.buffer.ByteBuf} (which might feel counterintuitive,
 * and different from {@link io.netty5.buffer.ByteBufUtil#indexOf} which returns the index of the first byte
 * of found sequence).
 * <br>
 ...
 * {@link io.netty5.buffer.ByteBuf}s). A new instance should be created with {@link AbstractSearchProcessorFactory} for
 ...
 * <br>
 * Example (given that the {@code haystack} is a {@link io.netty5.buffer.ByteBuf} containing "ABABAB" and
...
public abstract class AbstractSearchProcessorFactory implements SearchProcessorFactory {
     ...
     * {@link io.netty5.buffer.ByteBuf} on which the search is performed ({@code O(|haystack|)}).
     * Every byte of {@link io.netty5.buffer.ByteBuf} is processed only once, sequentially.
     *
