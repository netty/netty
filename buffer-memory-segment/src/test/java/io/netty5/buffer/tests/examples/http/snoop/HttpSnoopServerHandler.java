...
public class HttpSnoopServerHandler extends SimpleChannelInboundHandler<Object> {
            ...
            if (msg instanceof LastHttpContent) {
                ...
                if (!trailer.trailingHeaders().isEmpty()) {
                    ...
                    for (CharSequence name: trailer.trailingHeaders().names()) {
                        for (CharSequence value: trailer.trailingHeaders().values(name)) {
    ...
    private boolean writeResponse(HttpObject currentObj, ChannelHandlerContext ctx) {
        ...
        if (keepAlive) {
            ...
            response.headers().set(HttpHeaderNames.CONTENT_LENGTH, String.valueOf(response.payload().readableBytes()));
        ...
        // Encode the cookie.
        Iterator<HttpCookiePair> cookies = request.headers().getCookies().iterator();
        if (cookies.hasNext()) {
            // Reset the cookies if necessary.
            do {
                HttpCookiePair cookie = cookies.next();
                response.headers().addSetCookie(cookie.name(), cookie.value());
            } while (cookies.hasNext());
        } else {
            // Browser sent no cookie. Add some.
            response.headers().addSetCookie("key1", "value1");
            response.headers().addSetCookie("key2", "value2");
        }
    ...
    private static void send100Continue(ChannelHandlerContext ctx) {
        FullHttpResponse response = new DefaultFullHttpResponse(HTTP_1_1, CONTINUE, ctx.bufferAllocator().allocate(0));
        ctx.write(response);
    }
