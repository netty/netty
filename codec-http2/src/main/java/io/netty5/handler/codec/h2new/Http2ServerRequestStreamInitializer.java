...
final class Http2ServerRequestStreamInitializer extends ChannelInitializer<Http2StreamChannel> {
    ...
    Http2ServerRequestStreamInitializer(SensitivityDetector headerSensitivityDetector,
    ...
    protected void initChannel(Http2StreamChannel stream) throws Exception {
        ...
        final Http2RequestStreamFramesValidator framesValidator = new Http2RequestStreamFramesValidator(true);
        if (defaultDistributor != null) {
            stream.pipeline().addLast(new RequestStreamFlowControlFrameInspector(stream.streamId(), true,
