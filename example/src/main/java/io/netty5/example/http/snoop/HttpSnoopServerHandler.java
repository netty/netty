...
public class HttpSnoopServerHandler extends SimpleChannelInboundHandler<Object> {
    ...
    private boolean writeResponse(HttpObject currentObj, ChannelHandlerContext ctx) {
        ...
        // Encode the cookie.
        Iterable<HttpCookiePair> cookies = request.headers().getCookies();
        boolean hadCookies = false;
        // Reset the cookies if necessary.
        for (HttpCookiePair cookie: cookies) {
            response.headers().addSetCookie(new DefaultHttpSetCookie(cookie.name(), cookie.value()));
            hadCookies = true;
        }
        if (!hadCookies) {
            // Browser sent no cookie. Add some.
            response.headers().addSetCookie(new DefaultHttpSetCookie("key1", "value1"));
            response.headers().addSetCookie(new DefaultHttpSetCookie("key2", "value2"));
        }
