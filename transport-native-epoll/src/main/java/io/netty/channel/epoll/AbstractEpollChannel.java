...
abstract class AbstractEpollChannel extends AbstractChannel implements UnixChannel {
    ...
    AbstractEpollChannel(Socket fd, int flag) {
        this(null, fd, flag, false, null);
    }

    AbstractEpollChannel(Channel parent, Socket fd, int flag, boolean active, InetSocketAddress remote) {
        super(parent);
        ...
        boolean doFinishConnect() throws Exception {
            if (fd().finishConnect()) {
                ...
                if (requestedRemoteAddress instanceof InetSocketAddress) {
                    remote = computeRemoteAddr((InetSocketAddress) requestedRemoteAddress, fd().remoteAddress());
                }
    ...
    protected boolean doConnect(SocketAddress remoteAddress, SocketAddress localAddress) throws Exception {
        ...
        if (connected) {
            remote = remoteSocketAddr == null ?
                    remoteAddress : computeRemoteAddr(remoteSocketAddr, fd().remoteAddress());
        }
    ...
    private boolean doConnect0(SocketAddress remote) throws Exception {
        ...
        try {
            boolean connected = fd().connect(remote);
            if (!connected) {
                setFlag(Native.EPOLLOUT);
            }
            success = true;
