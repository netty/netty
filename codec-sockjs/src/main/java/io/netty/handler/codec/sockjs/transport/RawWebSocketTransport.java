...
public class RawWebSocketTransport extends ChannelInboundHandlerAdapter {
    ...
    private static boolean checkRequestHeaders(final ChannelHandlerContext ctx, final HttpRequest req) {
        ...
        if (connectHeader != null && "keep-alive".equalsIgnoreCase(connectHeader)) {
            req.headers().set(CONNECTION, HttpHeaderValues.UPGRADE);
            connectHeader = HttpHeaderValues.UPGRADE.toString();
        }
        if (connectHeader == null || !"upgrade".equalsIgnoreCase(connectHeader)) {
            ctx.writeAndFlush(responseFor(req)
                    .badRequest()
    ...
    private void handleHttpRequest(final ChannelHandlerContext ctx, FullHttpRequest req) {
        try {
            ...
            ctx.channel().attr(REQUEST_KEY).set(req);
            ...
            if (handshaker == null) {
                ...
                handshaker.handshake(ctx.channel(), req).addListener(new HandshakeFutureListener(ctx));
            }
        } finally {
            req.release();
        }
    ...
    private class HandshakeFutureListener implements ChannelFutureListener {
        private final ChannelHandlerContext ctx;

        HandshakeFutureListener(final ChannelHandlerContext ctx) {
            this.ctx = ctx;
        }
        ...
        public void operationComplete(final ChannelFuture future) throws Exception {
            if (future.isSuccess()) {
                ...
                service.onOpen(new SockJsSessionContext() {
                    ...
                    public void close() {
                        service.onClose();
                        handshaker.close(ctx.channel(), new CloseWebSocketFrame(3000, "Go away!"));
                    }
                    ...
                    public String sessionId() {
                        // a Raw WebSocket does not support SockJS sessions
                        return "none";
                    }
                });
            }
    ...
    public void exceptionCaught(final ChannelHandlerContext ctx, final Throwable cause) throws Exception {
        if (cause instanceof WebSocketHandshakeException) {
            final HttpRequest request = ctx.channel().attr(REQUEST_KEY).get();
