 ...
 */
final class WebSocketSessionState implements SessionState {
    ...
    public void onClose() {
        ...
        if (isWritable(channel)) {
            if (logger.isDebugEnabled()) {
                logger.debug("Writing SockJS GO_AWAY_FRAME");
                logger.debug("Writing CloseWebSocketFrame(1000, 'Normal closure'");
            }
            channel.write(GO_AWAY_FRAME.duplicate());
