 ...
 * {@link HttpHeaderUtil#getContentLength(HttpMessage, long)} which will set the lenght
...
public class WebSocketHAProxyHandshaker extends WebSocketServerHandshaker00 {
    ...
    protected FullHttpResponse newHandshakeResponse(FullHttpRequest req, HttpHeaders headers) {
        ...
        if (isHixie76) {
            ...
            key1 = req.headers().getAndConvert(SEC_WEBSOCKET_KEY1);
            key2 = req.headers().getAndConvert(SEC_WEBSOCKET_KEY2);
        } else {
    ...
    public static boolean isHAProxyReqeust(final FullHttpRequest request) {
        final String version = request.headers().getAndConvert(SEC_WEBSOCKET_VERSION);
