...
public final class JsonConverter {
    ...
    public static String encode(final String content) throws JsonMappingException {
        try {
            ...
            throw new JsonMappingException("content must be a JSON Array but was : " + content, e);
        }
    }
        ...
        for (int i = 0; i < count; i++) {
            ...
            switch(ch) {
                ...
                case '\\':
                    ...
                    break;
                case '\b':
                    escapeAndWriteByte(buffer, 'b');
                    break;
                case '\f':
                    escapeAndWriteByte(buffer, 'f');
                    break;
                case '\n':
                    escapeAndWriteByte(buffer, 'n');
                    break;
                case '\r':
                    escapeAndWriteByte(buffer, 'r');
                    break;
                case '\t':
                    escapeAndWriteByte(buffer, 't');
                    break;
                default:
                    if (shouldEscape((char) ch)) {
                        escapeAndPad(buffer, 'u');
                    } else {
                        buffer.writeByte(ch);
                    }
            }
    ...
    public static boolean isControlChar(final char c) {
        return isControlCharacter(c) || isFormatControlCharacter(c);
    }
    ...
    public static boolean isFormatControlCharacter(final char c) {
        return c == '\u200C' || c == '\u200D' || isFormatOtherCharacter(c);
    }
    ...
    public static String escapeCharacters(final char[] value) {
        ...
        for (char ch : value) {
            if (shouldEscape(ch)) {
                ...
                buffer.append(hex.toLowerCase(Locale.ENGLISH));
            } else {
                buffer.append(ch);
            }
        }
    ...
    private static ByteBuf escapeAndWriteByte(final ByteBuf buffer, final char ch) {
        return buffer.writeByte('\\').writeByte(ch);
    }

    private static void escapeAndPad(final ByteBuf buffer, final char ch) {
        final String hex = Integer.toHexString(ch);
        escapeAndWriteByte(buffer, ch);
        for (int k = 0; k < 4 - hex.length(); k++) {
            buffer.writeByte('0');
        }
        buffer.writeBytes(hex.toLowerCase(Locale.ENGLISH).getBytes());
    }

    /**
     * Taken from
     * <a href="https://github.com/FasterXML/jackson-docs/wiki/JacksonSampleQuoteChars">JacksonSampleQuoteChars</a>
     * with a few minor modifictions.
     */
    private static class StringSerializer extends JsonSerializer<String> {
        ...
        throws IOException {
            ...
            switch (status) {
                ...
                default:
                    // no action for any other statuses than the ones above
            }
            gen.writeRaw('"');
