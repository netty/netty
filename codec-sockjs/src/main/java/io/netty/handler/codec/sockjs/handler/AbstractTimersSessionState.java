...
abstract class AbstractTimersSessionState implements SessionState {
    ...
    AbstractTimersSessionState(final ConcurrentMap<String, SockJsSession> sessions,
                                         final SockJsSession session) {
        checkNotNull(sessions, "sessions");
    ...
    public void onClose() {
        ...
        final Channel channel = getSendingContext().channel();
    ...
    public void onSockJSServerInitiatedClose() {
        session.onClose();
        session.setInuse(false);
        final SockJsSession removed = sessions.remove(sockJsSession().sessionId());
        if (logger.isDebugEnabled()) {
    ...
    private final class SessionRemover implements Runnable {
