 ...
 *
 * The first poll request will open a session and the {@link ChannelHandlerContext} for
 ...
 *
 */
final class PollingSessionState extends AbstractTimersSessionState {
    ...
    public ChannelHandlerContext getSendingContext() {
        final ChannelHandlerContext openContext = sockJsSession().openContext();
        return openContext == null ? sockJsSession().connectionContext() : openContext;
    }
    ...
    public boolean isInUse() {
        return sockJsSession().connectionContext().channel().isActive() || sockJsSession().inuse();
    }
    ...
    Runnable createHeartbeater(final ChannelHandlerContext ignored) {
        return new PollingHeartbeater(sockJsSession());
    }
    ...
    private static final class PollingHeartbeater implements Runnable {
