...
public class SessionHandler extends ChannelInboundHandlerAdapter {
    ...
    private static final CloseFrame GO_AWAY_FRAME = new CloseFrame(3000, "Go away!");

    private final SessionState sessionState;

    public SessionHandler(final SessionState sessionState) {
        Arguments.checkNotNull(sessionState, "sessionState");
    ...
    private final class DefaultSockJsSessionContext implements SockJsSessionContext {
        ...
        public void close() {
            ...
            final Channel channel = sessionState.getSendingContext().channel();
            if (isWritable(channel)) {
                final CloseFrame closeFrame = GO_AWAY_FRAME.duplicate();
                if (logger.isDebugEnabled()) {
