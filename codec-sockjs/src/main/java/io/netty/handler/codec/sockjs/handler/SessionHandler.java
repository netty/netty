 ...
 *
 * For every connection received a new SessionHandler will be created and added to the pipeline.
 *
...
public class SessionHandler extends ChannelInboundHandlerAdapter {
    ...
    private static final OpenFrame OPEN_FRAME = new OpenFrame();

    private final SessionState sessionState;

    SessionHandler(final SessionState sessionState) {
        ObjectUtil.checkNotNull(sessionState, "sessionState");
    ...
    private void handleSession(final ChannelHandlerContext ctx) throws Exception {
        ...
        case CLOSED:
            ...
            break;
        default:
            logger.info("Unknown session state: " + sessionState.getState());
            sessionClosed(ctx);
            break;
        }
    }
    ...
    private void sessionOpen(final ChannelHandlerContext ctx) {
    ...
    private static void sessionInterrupted(final ChannelHandlerContext ctx) {
        writeCloseFrame(ctx, 1002, "Connection interrupted");
    }

    private void sessionClosed(final ChannelHandlerContext ctx) {
    ...
    private static void writeOpenFrame(final ChannelHandlerContext ctx) {
        ctx.channel().writeAndFlush(OPEN_FRAME.duplicate());
    }

    private final class DefaultSockJsSessionContext implements SockJsSessionContext {
