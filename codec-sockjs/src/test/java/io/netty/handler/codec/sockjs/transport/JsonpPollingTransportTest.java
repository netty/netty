...
public class JsonpPollingTransportTest {
    ...
    private static FullHttpResponse writeFrame(final Frame frame) {
        return writeFrame(frame, true, SockJsConfig.withPrefix("/echo").cookiesNeeded().build());
    }

    private static FullHttpResponse writeFrame(final Frame frame, final boolean withCallback) {
        return writeFrame(frame, withCallback, SockJsConfig.withPrefix("/echo").cookiesNeeded().build());
    }
    ...
    private static FullHttpResponse writeFrame(final Frame frame,
        ...
        final JsonpPollingTransportInbound inbound = new JsonpPollingTransportInbound();
        final JsonpPollingTransportOutbound outbound = new JsonpPollingTransportOutbound(config, request);
        final EmbeddedChannel ch = removeLastInboundMessageHandlers(new EmbeddedChannel(inbound, outbound));
