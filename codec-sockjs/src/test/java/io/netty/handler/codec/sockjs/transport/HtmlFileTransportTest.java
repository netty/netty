...
public class HtmlFileTransportTest {
    ...
    private static EmbeddedChannel newStreamingChannel(final SockJsConfig config, final String path) {
        ...
        final HtmlFileTransportInbound inbound = new HtmlFileTransportInbound();
        final HtmlFileTransportOutbound outbound = new HtmlFileTransportOutbound(config, request);
        return removeLastInboundMessageHandlers(new EmbeddedChannel(inbound, outbound));
    }
