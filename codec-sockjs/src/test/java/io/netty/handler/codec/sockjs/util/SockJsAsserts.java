...
public final class SockJsAsserts {
    ...
    public static void assertOrigin(final HttpResponse response, final String origin) {
        ...
        if (!origin.equals("*")) {
            assertThat(headers.get(ACCESS_CONTROL_ALLOW_CREDENTIALS), equalTo("true"));
        } else {
    ...
    public static void assertAllowCredentials(final HttpResponse repsonse, final boolean expected) {
        if (expected) {
            assertThat(repsonse.headers().get(ACCESS_CONTROL_ALLOW_CREDENTIALS), is("true"));
        } else {
            assertThat(repsonse.headers().get(ACCESS_CONTROL_ALLOW_CREDENTIALS), is(CoreMatchers.nullValue()));
        }
    }
