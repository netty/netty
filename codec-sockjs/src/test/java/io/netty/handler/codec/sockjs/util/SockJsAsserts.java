...
public final class SockJsAsserts {
    ...
    public static void assertWebSocketUpgradeResponse(final EmbeddedChannel ch) throws Exception {
        ...
        assertThat(connection.equalsIgnoreCase(HttpHeaderValues.UPGRADE.toString()), is(true));
    ...
    public static void assertWebSocketGoAwayFrame(final EmbeddedChannel ch) {
        final TextWebSocketFrame closeFrame = ch.readOutbound();
        System.out.println(closeFrame.content().toString(UTF_8));
        assertThat(closeFrame.content().toString(UTF_8), equalTo("c[3000,\"Go away!\"]"));
        release(closeFrame);
    }

    public static void assertRawWebSocketGoAwayFrame(final EmbeddedChannel ch) {
        final CloseWebSocketFrame closeFrame = ch.readOutbound();
        assertThat(closeFrame.statusCode(), is(3000));
        assertThat(closeFrame.reasonText(), is("Go away!"));
        release(closeFrame);
    }
    ...
    public static void assertChannelFinished(final EmbeddedChannel... channels) {
        for (EmbeddedChannel ch : channels) {
            assertThat(ch.outboundMessages().isEmpty(), is(true));
        }
    }
    ...
    private static String generatePrelude(final int size, final boolean newLine) {
        final int chars = size - (newLine ? 2 : 1);
