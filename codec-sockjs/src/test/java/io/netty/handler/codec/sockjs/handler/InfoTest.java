...
public class InfoTest {
    ...
    public void contentTypeHeader() throws Exception {
        assertThat(headersFromInfo().get(CONTENT_TYPE), equalTo("application/json; charset=UTF-8"));
    }

    @Test
    public void expiresHeader() throws Exception {
        assertThat(headersFromInfo().get(EXPIRES), is(nullValue()));
    }

    @Test
    public void lastModifiedHeader() throws Exception {
        assertThat(headersFromInfo().get(LAST_MODIFIED), is(nullValue()));
    }

    @Test
    public void setCookieHeader() throws Exception {
        assertThat(headersFromInfo().get(SET_COOKIE), is(nullValue()));
    }

    @Test
    public void contentLenghtHeader() throws Exception {
        assertThat(headersFromInfo().get(CONTENT_LENGTH), is(notNullValue()));
    }

    @Test
    public void cacheControlHeader() throws Exception {
        final String cacheControl = headersFromInfo().get(CACHE_CONTROL);
