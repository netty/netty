...
public abstract class AbstractEventExecutor extends AbstractExecutorService implements EventExecutor {
    ...
    private static final InternalLogger logger =
            InternalLoggerFactory.getInstance(AbstractEventExecutor.class);
    ...
    private static final AtomicIntegerFieldUpdater<AbstractEventExecutor> STATE_UPDATER;

    private static final long threadOffset;

    static {
        AtomicIntegerFieldUpdater<AbstractEventExecutor> updater =
                PlatformDependent.newAtomicIntegerFieldUpdater(AbstractEventExecutor.class, "state");
        if (updater == null) {
            updater = AtomicIntegerFieldUpdater.newUpdater(AbstractEventExecutor.class, "state");
        }
        STATE_UPDATER = updater;

        try {
            threadOffset =
                    PlatformDependent.objectFieldOffset(AbstractEventExecutor.class.getDeclaredField("thread"));
     ...
     *                          the executor thread.
     */
    protected AbstractEventExecutor(EventExecutorGroup parent, Executor executor, boolean addTaskWakesUp) {
        if (executor == null) {
     ...
     * {@link LinkedBlockingQueue} but if your sub-class of {@link AbstractEventExecutor} will not do any blocking
    ...
    protected void cleanupAndTerminate(boolean success) {
        for (;;) {
            int oldState = STATE_UPDATER.get(AbstractEventExecutor.this);
            if (oldState >= ST_SHUTTING_DOWN || STATE_UPDATER.compareAndSet(
                    AbstractEventExecutor.this, oldState, ST_SHUTTING_DOWN)) {
                break;
            }
        }
        ...
        if (success && gracefulShutdownStartTime == 0) {
            logger.error("Buggy " + EventExecutor.class.getSimpleName() + " implementation; " +
                    AbstractEventExecutor.class.getSimpleName() + ".confirmShutdown() must be called " +
        ...
        try {
            ...
            try {
                ...
                STATE_UPDATER.set(AbstractEventExecutor.this, ST_TERMINATED);
                threadLock.release();
    ...
    private static final class RunnableToCallableAdapter implements CallableEventExecutorAdapter<Void> {
