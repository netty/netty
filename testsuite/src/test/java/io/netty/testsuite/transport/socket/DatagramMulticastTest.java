...
public class DatagramMulticastTest extends AbstractDatagramTest {
    ...
    public void testMulticast(Bootstrap sb, Bootstrap cb) throws Throwable {
        ...
        sb.option(ChannelOption.IP_MULTICAST_IF, TestUtils.LOOPBACK_IF);
        sb.option(ChannelOption.SO_REUSEADDR, true);
        cb.option(ChannelOption.IP_MULTICAST_IF, TestUtils.LOOPBACK_IF);
        ...
        cc.joinGroup(groupAddress, TestUtils.LOOPBACK_IF).sync();

        sc.write(new DatagramPacket(Unpooled.copyInt(1), groupAddress)).sync();
        assertTrue(mhandler.await());

        // leave the group
        cc.leaveGroup(groupAddress, TestUtils.LOOPBACK_IF).sync();
