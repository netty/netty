...
/**
 * {@link EmbeddedChannel} completes writes synchronously, so it cannot by itself model bytes that a real transport
 * has accepted but not yet drained. Tests that need to simulate such a slow transport use this to drive the
 * package-private pending-outbound-bytes accounting on {@link ChannelOutboundBuffer} directly.
 */
