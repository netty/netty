...
final class SslUtils {
   ...
   static final TrustManager[] EMPTY_TRUST_MANAGERS = new TrustManager[0];
    ...
    static void handleHandshakeFailure(ChannelHandlerContext ctx, SSLSession session, Throwable cause, boolean notify) {
        // We have may haven written some parts of data before an exception was thrown so ensure we always flush.
        // See https://github.com/netty/netty/issues/3900#issuecomment-172481830
        ctx.flush();
        if (notify) {
            ctx.fireChannelInboundEvent(new SslHandshakeCompletionEvent(session, cause));
        }
    }
