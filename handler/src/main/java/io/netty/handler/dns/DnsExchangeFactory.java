...
public class DnsExchangeFactory {
	...
	private static final EventExecutorGroup executor = new DefaultEventExecutorGroup(
			2);
	private static final InternalLogger logger = InternalLoggerFactory
			.getInstance(DnsExchangeFactory.class);
			...
			public void run() {
				try {
					Class<?> configClass = Class
							.forName("sun.net.dns.ResolverConfiguration");
					Method open = configClass.getMethod("open");
					Method nameservers = configClass.getMethod("nameservers");
					Object instance = open.invoke(null);
					@SuppressWarnings("unchecked")
					List<String> list = (List<String>) nameservers
							.invoke(instance);
					for (String dns : list) {
					...
					logger.warn(
							"Failed to obtain system's DNS server addresses, using defaults only.",
							e);
				}
			}
		}.start();
	...
	public static boolean validAddress(byte[] address) {
		try {
			...
			Callable<List<ByteBuf>> callback = new DnsCallback<List<ByteBuf>>(
					-1, sendQuery(Resource.TYPE_A, "google.com", id, channel));
	 ...
	 * @throws InterruptedException
	 */
	protected static Channel channelForAddress(byte[] dnsServerAddress)
		...
		if ((channel = dnsServerChannels.get(dnsServerAddress)) != null) {
			...
			synchronized (dnsServerChannels) {
				if ((channel = dnsServerChannels.get(dnsServerAddress)) == null) {
					...
					dnsServerChannels.put(dnsServerAddress, channel = b
	 ...
	 * @throws InterruptedException
	 */
	public static <T> Future<T> lookup(String domain)
			throws UnknownHostException, SocketException, InterruptedException {
		return resolveSingle(domain, dnsServers.get(0), Resource.TYPE_A,
				Resource.TYPE_AAAA);
	}
	 ...
	 * @throws InterruptedException
	 */
	public static <T> Future<List<T>> lookup(String domain, Integer family)
			throws UnknownHostException, SocketException, InterruptedException {
		if (family != null && family != 4 && family != 6) {
			throw new IllegalArgumentException(
					"Family must be 4, 6, or null to indicate both 4 and 6.");
		}
		if (family == null) {
	 ...
	 * @throws InterruptedException
	 */
	public static <T> Future<T> resolveSingle(String domain,
			byte[] dnsServerAddress, int... types) throws UnknownHostException,
		...
		return executor.submit(new SingleResultCallback<T>(
				new DnsCallback<List<T>>(dnsServers.indexOf(dnsServerAddress),
						queries)));
	}
	 ...
	 * @throws InterruptedException
	 */
	public static <T extends List<?>> Future<T> resolve(String domain,
			byte[] dnsServerAddress, int... types) throws UnknownHostException,
			SocketException, InterruptedException {
		for (int i = 0; i < types.length; i++) {
			@SuppressWarnings("unchecked")
		...
		return executor.submit(new DnsCallback<T>(dnsServers
	 ...
	 * @throws InterruptedException
	 */
	public static Future<List<ByteBuf>> resolve4(String domain)
			throws UnknownHostException, SocketException, InterruptedException {
		return resolve(domain, dnsServers.get(0), Resource.TYPE_A);
	}
	 ...
	 * @throws InterruptedException
	 */
	public static Future<List<ByteBuf>> resolve6(String domain)
			throws UnknownHostException, SocketException, InterruptedException {
		return resolve(domain, dnsServers.get(0), Resource.TYPE_AAAA);
	}
	 ...
	 * @throws InterruptedException
	 */
	public static Future<List<MailExchangerRecord>> resolveMx(String domain)
			throws UnknownHostException, SocketException, InterruptedException {
	 ...
	 * @throws InterruptedException
	 */
	public static Future<List<ServiceRecord>> resolveSrv(String domain)
			throws UnknownHostException, SocketException, InterruptedException {
	 ...
	 * @throws InterruptedException
	 */
	public static Future<List<List<String>>> resolveTxt(String domain)
			throws UnknownHostException, SocketException, InterruptedException {
	 ...
	 * @throws InterruptedException
	 */
	public static Future<List<String>> resolveCname(String domain)
			throws UnknownHostException, SocketException, InterruptedException {
	 ...
	 * @throws InterruptedException
	 */
	public static Future<List<String>> resolveNs(String domain)
			throws UnknownHostException, SocketException, InterruptedException {
	 ...
	 * @throws InterruptedException
	 */
	public static Future<List<String>> reverse(ByteBuf ipAddress)
			throws UnknownHostException, SocketException, InterruptedException {
		...
		return resolve(domain.append("in-addr.arpa").toString(),
				dnsServers.get(0), Resource.TYPE_PTR);
	}
	...
	protected static void removeChannel(Channel channel) {
		synchronized (dnsServerChannels) {
			for (Iterator<Map.Entry<byte[], Channel>> iter = dnsServerChannels
