...
public class SslHandler extends ByteToMessageDecoder implements ChannelOutboundHandler {
    ...
    // Used in unwrapNonAppData(...) as input for unwrap(...). Using this static instance reduce objection
    // creation as Unpooled.EMPTY_BUFFER.nioBuffer() creates a new ByteBuffer everytime.
    private static final ByteBuffer EMPTY_DIRECT_BYTEBUFFER = ByteBuffer.allocateDirect(0);
    ...
    // Used if SslEngine.wrap(...) should be called with a ByteBuf that is only backed by one ByteBuffer to reduce
    // the object creation.
