...
jint netty5_unix_register(JNIEnv* env, const char* packagePrefix) {
    ...
    if (netty5_unix_limits_JNI_OnLoad(env, packagePrefix) == JNI_ERR) {
        goto error;
    }
    limitsOnLoadCalled = 1;

    if (netty5_unix_errors_JNI_OnLoad(env, packagePrefix) == JNI_ERR) {
        goto error;
    }
    errorsOnLoadCalled = 1;

    if (netty5_unix_filedescriptor_JNI_OnLoad(env, packagePrefix) == JNI_ERR) {
        goto error;
    }
    filedescriptorOnLoadCalled = 1;

    if (netty5_unix_socket_JNI_OnLoad(env, packagePrefix) == JNI_ERR) {
        goto error;
    }
    socketOnLoadCalled = 1;

    if (netty5_unix_buffer_JNI_OnLoad(env, packagePrefix) == JNI_ERR) {
        goto error;
    }
...
void netty5_unix_unregister(JNIEnv* env, const char* packagePrefix) {
    netty5_unix_limits_JNI_OnUnLoad(env, packagePrefix);
    netty5_unix_errors_JNI_OnUnLoad(env, packagePrefix);
    netty5_unix_filedescriptor_JNI_OnUnLoad(env, packagePrefix);
    netty5_unix_socket_JNI_OnUnLoad(env, packagePrefix);
