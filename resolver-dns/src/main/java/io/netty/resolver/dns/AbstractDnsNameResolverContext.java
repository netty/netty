...
abstract class AbstractDnsNameResolverContext<T> {
    ...
    AbstractDnsNameResolverContext(DnsNameResolver parent, String hostname, int port,
                                   Promise<T> promise) {
        this.parent = parent;
    ...
    final void onResponse(final DnsQuestion question, AddressedEnvelope<DnsResponse, InetSocketAddress> response) {
    ...
    private void finishResolve() {
        ...
        if (resolvedAddresses != null) {
            ...
            for (InternetProtocolFamily f: resolveAddressTypes) {
                switch (f) {
                case IPv4:
                    if (finishResolveWithIPv4(resolvedAddresses, port)) {
                        return;
                    }
                    break;
                case IPv6:
                    if (finishResolveWithIPv6(resolvedAddresses, port)) {
                        return;
                    }
                    break;
                }
    ...
    protected abstract boolean finishResolveWithIPv4(List<InetAddress> resolvedAddresses, int port);
    protected abstract boolean finishResolveWithIPv6(List<InetAddress> resolvedAddresses, int port);
