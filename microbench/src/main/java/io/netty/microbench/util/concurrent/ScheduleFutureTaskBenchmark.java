 ...
 * under the License.
 */
package io.netty.microbench.util.concurrent;
...
public class ScheduleFutureTaskBenchmark extends AbstractMicrobenchmark {
    ...
    private static final class BenchmarkEventLoop extends SingleThreadIoEventLoop {
        BenchmarkEventLoop(EventLoopGroup parent, Executor executor, IoHandlerFactory ioHandlerFactory) {
    ...
    private static final class BenchmarkEventLoopGroup extends MultiThreadIoEventLoopGroup {
        BenchmarkEventLoopGroup() {
            super(1, NioIoHandler.newFactory());
        }

        @Override
        protected EventLoop newChild(Executor executor, IoHandlerFactory ioHandlerFactory, Object... args) {
            return new BenchmarkEventLoop(this, executor, ioHandlerFactory);
        }
    }
    ...
    public static class ThreadState {
        ...
        BenchmarkEventLoop eventLoop;

        @Setup(Level.Trial)
        public void reset() {
            eventLoop = (BenchmarkEventLoop) new BenchmarkEventLoopGroup().next();
        }
