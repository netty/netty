...
mkdir -p "${MERGED_CONFIG_PATH}"

# Execute the netty build.
cd "${SCRIPT_PATH}/.."
mvn -DfailIfNoTests=false -Dtest=\!EpollDatagramMulticastIpv6WithIpv4AddrTest,\!ShadingIT -Dit.test=\!ShadingIT -DargLine.javaProperties="-Dio.netty.tryReflectionSetAccessible=true -agentlib:native-image-agent=config-output-dir=${COLLECTED_CONFIG_PATH}/{pid},experimental-conditional-config-part" package
