...
public class DnsResponseTest {
    ...
    public void readResponseTest() throws Exception {
        ...
        for (int i = 0; i < packets.length; i++) {
            ByteBuf packet = embedder.alloc().buffer(512).writeBytes(packets[i]);
            embedder.writeInbound(new DatagramPacket(packet, null));
            DnsResponse decoded = (DnsResponse) embedder.readInbound();
            packet.retain().readerIndex(0);
            DnsResponse expected = DnsResponseDecoder.decodeResponse(packet, embedder.alloc());
            Assert.assertEquals("Malformed response packet", decoded.content(), expected.content());
            ByteBuf raw = Unpooled.wrappedBuffer(packets[i]);
