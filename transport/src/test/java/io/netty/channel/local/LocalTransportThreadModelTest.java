...
public class LocalTransportThreadModelTest {
    ...
    public void testStagedExecution() throws Throwable {
        EventLoopGroup l = new DefaultEventLoopGroup(4, new BogoExecutorFactory("l"));
        EventExecutorGroup e1 = new DefaultEventExecutorGroup(4, new BogoExecutorFactory("e1"));
        EventExecutorGroup e2 = new DefaultEventExecutorGroup(4, new BogoExecutorFactory("e2"));
    ...
    private static final class BogoExecutorFactory implements ExecutorFactory {

        private final String namePrefix;

        BogoExecutorFactory(String namePrefix) {
            this.namePrefix = namePrefix;
        }

        @Override
        public Executor newExecutor(int parallelism) {
            return new BogoExecutor(namePrefix, parallelism);
        }

        private static final class BogoExecutor implements Executor {
            ...
            BogoExecutor(final String namePrefix, int parallelism) {
                numExecutors = parallelism * 2;
                executors = new ArrayList<Executor>(numExecutors);

                ThreadFactory threadFactory = new ThreadFactory() {
                    @Override
                    public Thread newThread(Runnable r) {
                        Thread thread = new Thread(r);
                        thread.setName(namePrefix + "-" + idx.incrementAndGet());
            ...
            public void execute(final Runnable command) {
                final Executor next = chooseNext();
                next.execute(command);
            }

            private Executor chooseNext() {
                int idx = rand.nextInt(numExecutors);
