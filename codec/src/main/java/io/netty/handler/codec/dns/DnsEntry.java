...
public class DnsEntry {
	...
	public DnsEntry(String name, int type, int dnsClass) {
		if (name == null || name.length() == 0) {
			throw new IllegalArgumentException(
					"name must not be null or left blank.");
		}
		if ((type & 0xffff) != type) {
			throw new IllegalArgumentException(
	...
	public String toString() {
		return new StringBuilder().append(getClass().getSimpleName())
				.append("(domain name: ").append(name).append(", type: ")
				.append(type).append(", class: ").append(dnsClass).append(')')
				.toString();
	}
	...
	public boolean equals(Object o) {
		...
		if (o instanceof DnsEntry) {
			...
			return other.name().equals(name) && other.type() == type
					&& other.dnsClass() == dnsClass;
		}
		return false;
	}
}
