...
public class DnsEntry {
	 ...
	 * RFC 2672
	 * DNAME creates an alias for a name and all its subnames, unlike CNAME, which aliases only the exact name in its label. Like the CNAME record, the DNS lookup will continue by retrying the lookup with the new name.
	 */
	 ...
	 * RFC 6698
	 * A record for DNS-based Authentication of Named Entities (DANE). RFC 6698 defines The TLSA DNS resource record is used to associate a TLS server certificate or public key with the domain name where the record is found, thus forming a 'TLSA certificate association'.
	 */
	 ...
	 * RFC 6844
	 * CA pinning, constraining acceptable CAs for a host/domain.
	 */
	public static final int TYPE_CAA = 0x0101;

	/**
	 * DNSSEC Trust Authorities record
	 * N/A
