...
/**
 * Custom caliper runner for {@link MetricsBenchmark}.
 */
public final class MetricsRunner {
     ...
     */
    public static void execute(final Class<? extends MetricsBenchmark> klaz)
            throws Exception {
        execute("WARMUP", klaz);
    ...
    public static Run execute(final String name,
            final Class<? extends MetricsBenchmark> klaz) throws Exception {

        final MetricsBenchmark booter = klaz.newInstance();
        ...
        for (final Map<String, String> vars : varsSet) {
            ...
            final MetricsBenchmark bench = (MetricsBenchmark) runner.getBenchmark();
