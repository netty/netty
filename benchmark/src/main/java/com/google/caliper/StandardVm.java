...
final class StandardVm extends Vm {
    ...
    public List<String> getVmSpecificOptions(final MeasurementType type,
        ...
        final List<String> result = Lists.newArrayList("-server", "-dsa",
                "-da", "-ea:io.netty...", "-Xms768m", "-Xmx768m",
                "-XX:MaxDirectMemorySize=768m", "-XX:+AggressiveOpts",
                "-XX:+UseBiasedLocking", "-XX:+UseFastAccessorMethods",
                "-XX:+UseStringCache", "-XX:+OptimizeStringConcat",
                "-XX:+HeapDumpOnOutOfMemoryError");

        if (type == MeasurementType.TIME) {
            Collections.addAll(result, "-XX:+PrintCompilation");
        } else {
            Collections.addAll(result, "-verbose:gc", "-Xbatch",
