 ...
 *
 */
public class BigIntegerDecoder extends ReplayingDecoder {

    @Override
    protected Object decode(ByteArrayBuffer buffer) throws Exception {
        int dataLength = buffer.readBE32();
        ByteArray data = buffer.read(dataLength);
        byte[] decoded = new byte[data.length()];
        data.get(data.firstIndex(), decoded);
