...
public final class SocksCmdRequest extends SocksRequest {
    ...
    public void encodeAsByteBuf(ChannelBuffer channelBuffer) {
        ...
        switch (addressType) {
            case IPv4: {
                channelBuffer.writeBytes(IPAddressUtil.textToNumericFormatV4(host));
            ...
            case DOMAIN: {
                ...
                channelBuffer.writeBytes(host.getBytes(CharsetUtil.US_ASCII));
                channelBuffer.writeShort(port);
                break;
            }

            case IPv6: {
                channelBuffer.writeBytes(IPAddressUtil.textToNumericFormatV6(host));
                channelBuffer.writeShort(port);
                break;
            }
        }
