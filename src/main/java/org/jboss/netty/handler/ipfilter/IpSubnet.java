...
public class IpSubnet implements IpSet, Comparable<IpSubnet> {
    ...
    public static void main(String[] args) {
        if (args.length != 0) {
            ...
            System.out.println("IpSubnet: " + ipSubnet.toString() + " from " +
                    ipSubnet.cidr.getBaseAddress() + " to " +
                    ipSubnet.cidr.getEndAddress() + " mask " +
                    ipSubnet.cidr.getMask());
            if (args.length > 1) {
                try {
                    System.out.println("Is IN: " + args[1] + " " +
                            ipSubnet.contains(args[1]));
                } catch (UnknownHostException e) {
                }
            }
        }
