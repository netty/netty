 ...
 *
 * @version $Rev: 983 $, $Date: 2009-03-04 15:13:39 +0100 (Wed, 04 Mar 2009) $
 *
 */
class NioUdpWorker implements Runnable 
{
    ...
    void register(final NioDatagramChannel channel, final ChannelFuture future)
    {
        ...
        synchronized (startStopLock)
        {
            if (!started)
            {
                ...
                try
                {
                    executor.execute(new ThreadRenamingRunnable(this, "New I/O server worker #" + bossId + "'-'" + id));
                    success = true;
                } 
                finally
                {
    ...
    static void write(final NioDatagramChannel channel, final boolean mightNeedWakeup) {
        /*
     ...
     * @return
     */
    private static boolean scheduleWriteIfNecessary(final NioDatagramChannel channel) 
    {
    ...
    private static void writeNow(final NioDatagramChannel channel, final int writeSpinCount) 
    {
        boolean open = true;
            ...
            for (;;) 
            {
                ...
                try 
                {
                    for (int i = writeSpinCount; i > 0; i --) 
                    {
                        ...
                        int localWrittenBytes = channel.getDatagramChannel().send(buffer.toByteBuffer(), evt.getRemoteAddress());
    ...
    static void setInterestOps(final NioDatagramChannel channel, ChannelFuture future, int interestOps) 
    {
        final NioUdpWorker worker = channel.worker;
    ...
    private final class ChannelRegistionTask implements Runnable
    {
        ...
        public void run()
        {
            ...
            try
            {
                ...
                channel.getDatagramChannel().register(selector, SelectionKey.OP_READ | SelectionKey.OP_WRITE, channel);
                if (future != null)
                {
                    future.setSuccess();
                }
