...
public class XmlFrameDecoderTest {
    ...
    private final XmlFrameDecoder decoder = new XmlFrameDecoder(1024 * 1024);
    ...
    public void testDecodeWithFrameExceedingMaxLength() {
        final XmlFrameDecoder decoder = new XmlFrameDecoder(3);
        final ChannelHandlerContext ctx = createMock(ChannelHandlerContext.class);
        final Channel channel = createMock(Channel.class);
        final ChannelPipeline channelPipeline = createMock(ChannelPipeline.class);
        expect(ctx.getChannel()).andReturn(channel);
        expect(channel.getPipeline()).andReturn(channelPipeline);
        channelPipeline.sendUpstream(isA(DefaultExceptionEvent.class));
        replay(ctx, channel, channelPipeline);

        final ChannelBuffer buffer = fromString("<v/>");
        final Object result = decoder.decode(ctx, channel, buffer);
    ...
    public void testDecodeWithInvalidInput() {
        ...
        final ChannelBuffer buffer = fromString("invalid XML");
        final Object result = decoder.decode(ctx, channel, buffer);
        assertNull("result", result);
        verify(ctx);
        verify(channel);
    ...
    public void testDecodeWithInvalidContentBeforeXml() {
        ...
        final ChannelBuffer buffer = fromString("invalid XML<xxx/>");
        final Object result = decoder.decode(ctx, channel, buffer);
        assertNull("result", result);
        verify(ctx);
        verify(channel);
    ...
    public void testDecodeShortValidXmlWithLeadingWhitespace01() {
        final ChannelBuffer buffer = fromString("   <xxx/>");
        final Object result = decoder.decode(null, null, buffer);
        assertNotNull("result", result);
        assertEquals("s", "<xxx/>", fromBuffer(result));
    }

    @Test
    public void testDecodeShortValidXmlWithLeadingWhitespace02() {
        final ChannelBuffer buffer = fromString("  \n\r \t<xxx/>\t");
        final Object result = decoder.decode(null, null, buffer);
        assertNotNull("result", result);
        assertEquals("s", "<xxx/>", fromBuffer(result));
    }
    ...
    public void testDecodeWithTwoMessages() {
        final ChannelBuffer buffer = fromString("<root xmlns=\"http://www.acme.com/acme\" status=\"loginok\" timestamp=\"1362410583776\"/>\n" +
                "\n" +
        ...
        final Object result = decoder.decode(null, null, buffer);
        assertNotNull("result", result);
        assertEquals("s", "<root xmlns=\"http://www.acme.com/acme\" status=\"loginok\" timestamp=\"1362410583776\"/>", fromBuffer(result));
    }

    @Test
    public void testDecodeWithSampleXml() {
        for (final String xmlSample : xmlSamples) {
            testDecodeWithXml(xmlSample);
        }
    }

    private void testDecodeWithXml(String xml) {
        final ChannelBuffer buffer = fromString(xml);
        final Object result = decoder.decode(null, null, buffer);
        assertNotNull("result", result);
        assertEquals("s", xml, fromBuffer(result));
    }

    private ChannelBuffer fromString(String in) {
        return new ByteBufferBackedChannelBuffer(ByteBuffer.wrap(in.getBytes()));
    }

    private String fromBuffer(Object buffer) {
        assertTrue("buffer is instance of ChannelBuffer", buffer instanceof ChannelBuffer);
        return ((ChannelBuffer) buffer).toString(Charset.forName("UTF-8"));
    }
