...
public abstract class AbstractChannelBufferTest {
    ...
    public void testRandomHeapBufferTransfer() {
        ...
        ChannelBuffer value = new HeapChannelBuffer(valueContent);
        for (int i = 0; i < buffer.capacity() - BLOCK_SIZE + 1; i += BLOCK_SIZE) {
        ...
        ChannelBuffer expectedValue = new HeapChannelBuffer(expectedValueContent);
        for (int i = 0; i < buffer.capacity() - BLOCK_SIZE + 1; i += BLOCK_SIZE) {
    ...
    public void testRandomDirectBufferTransfer() {
        ...
        ChannelBuffer value = ChannelBuffers.directBuffer(BLOCK_SIZE * 2);
        for (int i = 0; i < buffer.capacity() - BLOCK_SIZE + 1; i += BLOCK_SIZE) {
        ...
        random.setSeed(seed);
        ChannelBuffer expectedValue = ChannelBuffers.directBuffer(BLOCK_SIZE * 2);
        for (int i = 0; i < buffer.capacity() - BLOCK_SIZE + 1; i += BLOCK_SIZE) {
            random.nextBytes(tmp);
    ...
    public void testDiscardReadBytes() {
        ...
        ChannelBuffer copy = ChannelBuffers.copiedBuffer(buffer);
